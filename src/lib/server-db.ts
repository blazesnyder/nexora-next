...
function readDb(): StoreShape {
  try {
    if (!fs.existsSync(DB_FILE)) return { posts: [], users: [], settings: { siteName: "Nexora", siteDescription: "AI, Gaming, Esports & Tech", postsPerPage: 9, enableComments: false } }
...
export function seedInitialData(posts: AdminPost[]) {
  ...
  if (db.posts.length === 0) {
    db.posts = posts
    db.users = [{ id: "1", email: "admin@nexora.com", name: "Admin", roles: ["Full Access", "Editor", "Author"], createdAt: "2026-01-01T00:00:00.000Z" }]
    writeDb(db)
  }
}
